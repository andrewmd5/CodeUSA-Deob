...
import org.codeusa.cache.stream.RSRandomAccessFile;
...
public class FileStore {
	...
	private final ByteBuffer tempBuffer = ByteBuffer
	...
	public FileStore(final int id, final String path) {
		this.id = id;
		try {
			this.data = new RSRandomAccessFile(path + "main_file_cache.dat2",
					"r");
			this.index = new RSRandomAccessFile(path + "main_file_cache.idx"
					+ id, "r");
	...
	public synchronized byte[] get(final int fileId) {
		try {
			if (((fileId * IDX_BLOCK_LEN) + IDX_BLOCK_LEN) > indexChannel
					.size()) {
				return null;
			}
			...
			if ((block <= 0)
					|| (block > (dataChannel.size() / TOTAL_BLOCK_LEN))) {
				return null;
			}
			...
			while (remaining > 0) {
				...
				final int blockSize = remaining > BLOCK_LEN ? BLOCK_LEN
						: remaining;
				tempBuffer.position(0).limit(blockSize + BLOCK_HEADER_LEN);
				dataChannel.read(tempBuffer, block * TOTAL_BLOCK_LEN);
				tempBuffer.flip();

				final int currentFile = tempBuffer.getShort() & 0xffff;
				final int currentChunk = tempBuffer.getShort() & 0xffff;
				final int nextBlock = getMediumInt(tempBuffer);
				final int currentIndex = tempBuffer.get() & 0xff;

				if ((fileId != currentFile) || (chunk != currentChunk)
						|| (id != currentIndex)) {
					return null;
				}
				if ((nextBlock < 0)
						|| (nextBlock > (dataChannel.size() / TOTAL_BLOCK_LEN))) {
					return null;
				}
