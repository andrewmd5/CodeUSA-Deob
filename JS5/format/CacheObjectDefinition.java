...
public class CacheObjectDefinition {
	...
	private void readValues(final RSByteBuffer stream, final int opcode)
			throws IOException {
										...
										if (opcode == 27) {
											...
											if (opcode != 39) {
												if ((opcode < 30)
																															...
																															if (opcode == 102) {
																																...
																																if (opcode == 105) {
																																	...
																																	anInt3851 = stream
																																		...
																																		&& (opcode < 155)) {
																																	objectOptions[opcode - 150] = stream
																																			.readRS2String();
																																} else if (opcode != 160) {
																																	if (opcode == 162) {
																																		aByte3912 = (byte) 3;
													...
													objectOptions[opcode - 30] = stream
															.readRS2String();
												}
											} else {
												stream.readByte();
	...
	private void method3297(final RSByteBuffer stream) throws IOException {
	...
	private void readValueLoop(final RSByteBuffer stream) throws IOException {
		for (;;) {
			final int opcode = stream.readByte() & 0xff;
			if (opcode == 0) {
				break;
			}
	...
	// CacheObjectChildDefinition class281 =
	// CacheObjectChildDefinition.valueOf(i);
	...
	public boolean containsOption(final String o) {
		...
		for (final String option : objectOptions) {
			if (option == null) {
				continue;
			}
	...
	public static CacheObjectDefinition forId(final int objectID) {
		...
		if (is != null) {
			try {
				objectDef.readValueLoop(new RSByteBuffer(
		...
		if (objectDef.name.equals("Counter")) {
			objectDef.clippingFlag = false;
			objectDef.solid = true;
			objectDef.actionCount = 2;
		}
