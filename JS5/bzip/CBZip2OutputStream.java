...
public class CBZip2OutputStream extends OutputStream implements BZip2Constants {
	...
	protected static void hbMakeCodeLengths(final char[] len, final int[] freq,
	...
	public CBZip2OutputStream(final OutputStream inStream, int inBlockSize)
			throws IOException {
		block = null;
		quadrant = null;
		zptr = null;
	...
	private void hbAssignCodes(final int[] code, final char[] length,
	...
	private void bsSetStream(final OutputStream f) {
		bsStream = f;
		bsLive = 0;
		bsBuff = 0;
		bytesOut = 0;
	...
	private void bsPutUChar(final int c) throws IOException {
		bsW(8, c);
	}
	...
	private void bsPutIntVS(final int numBits, final int c) throws IOException {
		bsW(numBits, c);
	}
	...
	private void simpleSort(final int lo, final int hi, final int d) {
		int i, j, h, bigN, hp;
		int v;
	...
	private void qSort3(final int loSt, final int hiSt, final int dSt) {
			...
			for (i = 0; i <= 255; i++) {
				...
				for (j = 0; j <= 255; j++) {
					...
					if (!((ftab[sb] & SETMASK) == SETMASK)) {
						final int lo = ftab[sb] & CLEARMASK;
				...
				if (i < 255) {
					...
					final int bbSize = (ftab[(ss + 1) << 8] & CLEARMASK)
							- bbStart;
					int shifts = 0;
					...
					for (j = 0; j < bbSize; j++) {
						final int a2update = zptr[bbStart + j];
	 ...
	 * <= 20.
	 */
	private final int[] incs = { 1, 4, 13, 40, 121, 364, 1093, 3280, 9841,
			29524, 88573, 265720, 797161, 2391484 };

	private void allocateCompressStructures() {
		final int n = baseBlockSize * blockSize100k;
