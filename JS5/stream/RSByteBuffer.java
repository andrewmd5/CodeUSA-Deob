...
public class RSByteBuffer extends DataInputStream {
	...
	public RSByteBuffer(final InputStream in) {
		super(in);
	}
	...
	public int method2225(final int i) throws IOException {
	...
	public int readShortLEA() throws IOException {
		final byte bl = readByte();
		final byte bh = readByte();
	...
	public int read24BitInt() throws IOException {
		return ((read() & 0xff) << 16) + ((read() & 0xff) << 8)
				+ (read() & 0xff);
	}
