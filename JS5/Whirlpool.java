...
public class Whirlpool {
	...
	public static byte[] whirlpool(final byte[] data, final int off,
			final int len) {
		byte source[];
		if (off <= 0) {
			source = data;
	...
	public void NESSIEadd(final byte[] source, long sourceBits) {
		...
		// to 8 bits).
		final int sourceGap = (8 - ((int) sourceBits & 7)) & 7; // space on
		// source[sourcePos].
		final int bufferRem = bufferBits & 7; // occupied bits on
		// buffer[bufferPos].
		int b;
