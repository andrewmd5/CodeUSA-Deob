...
public class XTEADecrypter {
	...
	public static byte[] decryptXTEA(final int[] keys, final byte[] data,
			final int offset, final int length) {
		final int qword_count = (length - offset) / 8;
		final XTEAStream x = new XTEAStream(data);
		x.offset = offset;
		for (int qword_pos = 0; qword_pos < qword_count; qword_pos++) {
			int dword_1 = x.readInt();
			int dword_2 = x.readInt();
			int const_1 = -957401312;
			final int const_2 = -1640531527;
			int run_count = 32;
			while (~run_count-- < -1) {
				dword_2 -= (((dword_1 >>> 5) ^ (dword_1 << 4)) + dword_1)
						^ (const_1 + keys[(const_1 >>> 11) & 0x56c00003]);
				const_1 -= const_2;
				dword_1 -= (((dword_2 >>> 5) ^ (dword_2 << 4)) - -dword_2)
	...
	private static class XTEAStream {
		...
		public XTEAStream(final byte[] data) {
			buffer = data;
			offset = 0;
		}

		public void writeInt(final int i_29_) {
