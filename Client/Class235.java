/* Class235 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

final class Class235 {
	Runnable aRunnable1752;
	static long aLong1753;
	int anInt1754;
	int anInt1755;
	private ha_Sub2 aHa_Sub2_1756;
	int anInt1757 = 0;
	boolean aBoolean1758;
	boolean aBoolean1759;
	Class111_Sub2 aClass111_Sub2_1760;
	int anInt1761;
	boolean aBoolean1762;
	int anInt1763;
	static int[] anIntArray1764 = null;
	int[] anIntArray1765;
	int[] anIntArray1766;
	Class12 aClass12_1767;
	int[] anIntArray1768;
	Class146_Sub1 aClass146_Sub1_1769;
	int[] anIntArray1770;
	int anInt1771;
	Class146_Sub1 aClass146_Sub1_1772;
	int[] anIntArray1773;
	Class146_Sub1 aClass146_Sub1_1774;
	int[] anIntArray1775;
	Class146_Sub1 aClass146_Sub1_1776;
	int[] anIntArray1777;
	Class146_Sub1 aClass146_Sub1_1778;
	int anInt1779;
	int[] anIntArray1780;
	int[] anIntArray1781;
	int[] anIntArray1782;
	int anInt1783;
	int[] anIntArray1784;
	int[] anIntArray1785;
	int[] anIntArray1786;
	Class146_Sub1 aClass146_Sub1_1787;
	int[] anIntArray1788;
	int[] anIntArray1789;
	Class146_Sub1 aClass146_Sub1_1790;
	int[] anIntArray1791;
	int[] anIntArray1792;
	Class146_Sub1 aClass146_Sub1_1793;
	int[] anIntArray1794;
	int[] anIntArray1795;
	int[] anIntArray1796;
	int[] anIntArray1797;
	Class146_Sub1 aClass146_Sub1_1798;
	float[] aFloatArray1799;
	Class146_Sub1 aClass146_Sub1_1800;

	final void method2888(final byte i) {
		try {
			if (i < -34) {
				aClass12_1767 = new Class12(aHa_Sub2_1756, this);
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ou.A(" + i + ')');
		}
	}

	final void method2889(final byte i, final Runnable runnable) {
		try {
			aRunnable1752 = runnable;
			if (i != 34) {
				aFloatArray1799 = null;
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, ("ou.C(" + i + ','
					+ (runnable != null ? "{...}" : "null") + ')'));
		}
	}

	public static void method2890(final int i) {
		do {
			try {
				anIntArray1764 = null;
				if (i == 64) {
					break;
				}
				aLong1753 = -27L;
			} catch (final RuntimeException runtimeexception) {
				throw Class64_Sub27.printError(runtimeexception, "ou.B(" + i
						+ ')');
			}
			break;
		} while (false);
	}

	Class235(final ha_Sub2 var_ha_Sub2) {
		anInt1755 = 0;
		anInt1754 = 0;
		aBoolean1762 = true;
		anInt1763 = 0;
		aBoolean1759 = false;
		aClass111_Sub2_1760 = new Class111_Sub2();
		anIntArray1766 = new int[Class146_Sub1.anInt4825];
		anIntArray1765 = new int[Class146_Sub1.anInt4825];
		anIntArray1770 = new int[10];
		anIntArray1781 = new int[64];
		anIntArray1777 = new int[10];
		anIntArray1785 = new int[10000];
		anIntArray1784 = new int[64];
		anIntArray1792 = new int[8];
		anIntArray1780 = new int[Class146_Sub1.anInt4825];
		anIntArray1768 = new int[10];
		anIntArray1789 = new int[10];
		aFloatArray1799 = new float[2];
		anIntArray1797 = new int[8];
		anIntArray1786 = new int[Class146_Sub1.anInt4825];
		anIntArray1795 = new int[64];
		anIntArray1782 = new int[10000];
		anIntArray1796 = new int[Class146_Sub1.anInt4825];
		anIntArray1791 = new int[8];
		anIntArray1794 = new int[Class146_Sub1.anInt4825];
		anIntArray1788 = new int[64];
		anIntArray1775 = new int[Class146_Sub1.anInt4825];
		try {
			aHa_Sub2_1756 = var_ha_Sub2;
			anInt1761 = -255 + aHa_Sub2_1756.anInt4484;
			aClass12_1767 = new Class12(var_ha_Sub2, this);
			aClass146_Sub1_1774 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1790 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1778 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1769 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1776 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1800 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1787 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1793 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1798 = new Class146_Sub1(aHa_Sub2_1756);
			aClass146_Sub1_1772 = new Class146_Sub1(aHa_Sub2_1756);
			anIntArray1773 = new int[Class146_Sub1.anInt4810];
			for (int i = 0; (i ^ 0xffffffff) > (Class146_Sub1.anInt4810 ^ 0xffffffff); i++) {
				anIntArray1773[i] = -1;
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ou.<init>("
					+ (var_ha_Sub2 != null ? "{...}" : "null") + ')');
		}
	}
}
