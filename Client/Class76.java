/* Class76 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

abstract class Class76 {
	ha_Sub3 aHa_Sub3_585;
	static Class28[][] aClass28ArrayArray586;
	static IncomingOpcode aClass58_587 = new IncomingOpcode(6, -2);
	static int anInt588;
	static IncomingOpcode aClass58_589 = new IncomingOpcode(69, 8);
	static byte[][] aByteArrayArray590;

	void method737(final int i) {
		do {
			try {
				if (i == 2899) {
					break;
				}
				method746(-41, 122, 8);
			} catch (final RuntimeException runtimeexception) {
				throw Class64_Sub27.printError(runtimeexception, "ew.J(" + i
						+ ')');
			}
			break;
		} while (false);
	}

	void method738(final int i) {
		do {
			try {
				if (i <= -45) {
					break;
				}
				method745((byte) 50);
			} catch (final RuntimeException runtimeexception) {
				throw Class64_Sub27.printError(runtimeexception, "ew.K(" + i
						+ ')');
			}
			break;
		} while (false);
	}

	abstract void method739(int i);

	void method740(final int i) {
		try {
			if (i >= -49) {
				method748(93, false);
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ew.N(" + i + ')');
		}
	}

	Class76(final ha_Sub3 var_ha_Sub3) {
		try {
			aHa_Sub3_585 = var_ha_Sub3;
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ew.<init>("
					+ (var_ha_Sub3 != null ? "{...}" : "null") + ')');
		}
	}

	void method741(final byte i) {
		try {
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ew.L(" + i + ')');
		}
	}

	abstract void method742(int i, int i_1_, Interface4 interface4);

	abstract void method743(int i, boolean bool);

	public static void method744(final int i) {
		try {
			aClass58_589 = null;
			aClass58_587 = null;
			if (i <= 72) {
				method744(-44);
			}
			aClass28ArrayArray586 = null;
			aByteArrayArray590 = null;
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ew.Q(" + i + ')');
		}
	}

	abstract boolean method745(byte i);

	abstract void method746(int i, int i_2_, int i_3_);

	void method747(final int i) {
		try {
			if (i != -25684) {
				return;
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ew.P(" + i + ')');
		}
	}

	abstract void method748(int i, boolean bool);

	void method749(final int i) {
		try {
			if (i != 8) {
				method739(112);
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "ew.M(" + i + ')');
		}
	}
}
