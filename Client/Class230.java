/* Class230 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

final class Class230 {
	static String[] aStringArray1721;
	static int[] anIntArray1722;
	long aLong1723;
	static int[] anIntArray1724;
	static int anInt1725 = 50;
	static int[] anIntArray1726;
	static OutgoingOpcode aClass171_1727;
	static int[] anIntArray1728;
	static int[] anIntArray1729;
	static int[] anIntArray1730;
	private ha_Sub3_Sub2 aHa_Sub3_Sub2_1731;
	static int anInt1732;

	static final void method2869(final int i) {
		try {
			for (Class98_Sub46_Sub8 class98_sub46_sub8 = ((Class98_Sub46_Sub8) Class33.aClass148_315
					.method2418(32)); class98_sub46_sub8 != null; class98_sub46_sub8 = ((Class98_Sub46_Sub8) Class33.aClass148_315
					.method2417(101))) {
				if (Class98_Sub21.method1179(class98_sub46_sub8.anInt5990, 255)) {
					Class280.method3324(class98_sub46_sub8, -32612);
				}
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "on.C(" + i + ')');
		}
	}

	static final void method2870(final byte i) {
		do {
			try {
				Class98_Sub37.aHa4185
						.xa((((Class98_Sub9.aClass98_Sub27_3856).aClass64_Sub19_4057
								.method630((byte) 125) * 0.1F) + 0.7F)
								* Class159.aFloat1254);
				Class98_Sub37.aHa4185.ZA(Class98_Sub46_Sub6.anInt5979,
						NPC.aFloat1150, Class215.aFloat1613,
						InputStream_Sub2.anInt29 << -1868059326,
						Class48.anInt410 << -1893384734,
						OutputStream_Sub2.anInt40 << 1516482466);
				Class98_Sub37.aHa4185
						.method1775(Class98_Sub46_Sub4.aClass48_5962);
				if (i == -74) {
					break;
				}
				anIntArray1726 = null;
			} catch (final RuntimeException runtimeexception) {
				throw Class64_Sub27.printError(runtimeexception, "on.D(" + i
						+ ')');
			}
			break;
		} while (false);
	}

	@Override
	protected final void finalize() throws Throwable {
		try {
			aHa_Sub3_Sub2_1731.method2082(0, aLong1723);
			super.finalize();
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception,
					"on.finalize(" + ')');
		}
	}

	static final void method2871(final int i) {
		try {
			Class246.method2962(false);
			final int i_1_ = Class98_Sub9.aClass98_Sub27_3856.aClass64_Sub18_4071
					.method627((byte) 120);
			do {
				if ((i_1_ ^ 0xffffffff) != -3) {
					if (i_1_ != 3) {
						break;
					}
				} else {
					Class98_Sub10_Sub39.method1121((byte) 17, Class265.aHa1974,
							Class98_Sub25.anInt4024, Class39_Sub1.anInt3593,
							100, 100);
					break;
				}
				Class362.method3926(-120, 2, Class39_Sub1.anInt3593,
						Class341.anInt2856, Class265.aHa1974,
						Class356.anInt3018, 2, Class98_Sub25.anInt4024);
			} while (false);
			if (i >= -30) {
				method2869(-126);
			}
			if (Class98_Sub9.aClass98_Sub27_3856.aClass64_Sub18_4071
					.method625((byte) -77)) {
				Class293.method3471(Class42_Sub3.aCanvas5361, 0);
			}
			if (Class265.aHa1974 != null) {
				Class98_Sub10_Sub34.method1104(99);
			}
			Class239.aBoolean1839 = Class98_Sub9.aClass98_Sub27_3856.aClass64_Sub18_4071
					.method627((byte) 122) != 0;
			za_Sub2.aBoolean6079 = Class98_Sub9.aClass98_Sub27_3856.aClass64_Sub18_4071
					.method625((byte) -58);
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "on.B(" + i + ')');
		}
	}

	public static void method2872(final byte i) {
		try {
			aClass171_1727 = null;
			anIntArray1728 = null;
			aStringArray1721 = null;
			if (i <= -70) {
				anIntArray1730 = null;
				anIntArray1729 = null;
				anIntArray1722 = null;
				anIntArray1726 = null;
				anIntArray1724 = null;
			}
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, "on.A(" + i + ')');
		}
	}

	Class230(final ha_Sub3_Sub2 var_ha_Sub3_Sub2, final long l, final int i) {
		try {
			aLong1723 = l;
			aHa_Sub3_Sub2_1731 = var_ha_Sub3_Sub2;
		} catch (final RuntimeException runtimeexception) {
			throw Class64_Sub27.printError(runtimeexception, ("on.<init>("
					+ (var_ha_Sub3_Sub2 != null ? "{...}" : "null") + ',' + l
					+ ',' + i + ')'));
		}
	}

	static {
		anIntArray1724 = new int[anInt1725];
		anIntArray1730 = new int[anInt1725];
		anIntArray1729 = new int[anInt1725];
		anIntArray1728 = new int[anInt1725];
		aStringArray1721 = new String[anInt1725];
		anIntArray1726 = new int[anInt1725];
		anIntArray1722 = new int[anInt1725];
		aClass171_1727 = new OutgoingOpcode(62, 1);
		anInt1732 = 1;
	}
}
